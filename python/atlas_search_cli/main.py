...
def handle_vector_search(args):
    ...
    if args.embedWithVoyage:
        ...
        voyage_model = args.voyageModel
        if not voyage_model:
            voyage_model = current_config.get('voyageModel')
        if not voyage_model:
            voyage_model = 'voyage-3.5'
        embedding = vo.embed([args.query], model=voyage_model).embeddings[0]
...
def main():
    ...
    vector_parser.add_argument('--voyageModel', type=str, help='The Voyage AI model to use for embedding.')
